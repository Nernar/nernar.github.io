...
Он содержит неизменяемый идентификатор и подготовленную вариацию, а также предоставляет методы для обработки состояний. Мне кажется, неплохим примером будет связать [создание топора](../items/tools.md#топор) и его использование на блоках древесины. Каким образом? Топор обтесывает древесину, это нужно реализовать:
...
После призыва предмет может быть изменен с помощью [Entity.setDroppedItem](/api/namespace/Entity#setDroppedItem), используя уникальный идентификатор существа. Воспользуйтесь для этого возвращаемым результатом метода или событиями.
