...
Понятие создания существ несколько отличается от всего остального, для этого не подойдет скрипт, куда удобнее использовать пакеты поведения и ресурсов. Эта статья описывает множество новых понятий, не описанных ранее, однако мы лично познакомимся с большинством из них. Начнем с основы нового моба, а уже позже перейдем к описанию его поведения и созданию визуальной части.
...
Следующим и последним шагом задания основного объекта описания, будет определение возможности размещения существа в мире. За это отвечают несколько параметров, дополните ваш объект `description` ими:
...
Большую роль в визуалиации всего существа берет на себя текстура. Для существ они размещаются в папке __textures/entities__ вашего пакета ресурсов, обычно используя идентификатор моба. К примеру, загрузите готовую текстуру Хранителя, разместив его по пути *textures/entities/keeper.png*:
...
В большинстве случаев одной текстуры будет достаточно, ее можно создать используя различные редакторы моделей.
    ...
    "animation_controllers": {
        "controller.animation.mymod.keeper": {
            ...
            "states": {
                ...
                // переход с состояние сна, мобу в этот момент попросту
        ...
        "minecraft:entity": {
            ...
            "components": {
                    ...
                    "potions": [
                        {
                            ...
                            "chance": 1, // наибольшая вероятность
                            "filters": [
                                {
                ...
                "minecraft:behavior.target_when_pushed": {
                    "priority": 10,
                    "percent_chance": 5 // шанс на определение цели
                },
                    ...
                    "gives": [
                        {
                            ...
                            "functions": [
                                {
                                    // случайно добавим око, либо его не будет
...
## Использование движком

Так как эта документация все же базируется на технологиях Inner Core, последующие шаги как для описания существ, так и в целом использования пакетов ресурсов и поведения будут описаны на [сторонних ресурсах](TODO). Майнкрафт не ограничивается лишь нашим движком, здесь еще немало интересных технологий для контента!
